...
exports.getActivityByUser = async(req, res, next) => {
    ...
    try {
        ...
        const activitiesFromCurrentUser = await Activity.find({ 'author.id': req.params.userId }).sort({ creationMoment: -1 }).limit(5);
        activities.push.apply(activities, activitiesFromCurrentUser)
...
exports.getActivityByWorkspace = async(req, res, next) => {
    ...
    try {
        ...
        const activities = await Activity.find({ 'workspace.id': req.params.workspaceId }).sort({ creationMoment: -1 }).limit(10);
        return res.status(200).json({
            message: "Activities fetched successfully!",
            activities: activities,
