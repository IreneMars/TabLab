...
exports.createReport = async(req, res, next) => {
    ...
    try {
        const tests = await Test.find({ _id: req.query.testId }).exec();
        if (tests.length !== 1) {
        ...
        const esquemas = await Esquema.find({ _id: tests[0].esquema }).exec();
        const configurations = await Configuration.find({ _id: { $in: tests[0].configurations } }).exec();
